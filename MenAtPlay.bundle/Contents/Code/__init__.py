...
CROPPER = r'CScript.exe "{0}/Plex Media Server/Plug-ins/MenAtPlay.bundle/Contents/Code/ImageCropper.vbs" "{1}" "{2}" "{3}" "{4}"'
THUMBOR = Prefs['thumbor'] + "/0x0:{0}x{1}/{2}"

XML_PATHS = {
        'Search': '//div[contains(@class, "sceneTour")]',
        'SearchTitle': '//div[@class="updateDetails"]/h4/a/text()',
        'SearchUrl': '//div[@class="updateDetails"]/h4/a/@href',
        'SiteTitle': '//div[@class="bookmark"]/h2/text()',
        'SiteSummary': '//div[@class="video-descr__content"]/article',
        'ReleaseDate' : '//span[@class="availdate"]/text()',
        'SiteArts': '//video[@id="moviePlayerTour"]/@poster',
        'CastMembers': '//span[@class="tour_update_models"]/a',
        'CastMembersName': '//span[@class="tour_update_models"]/a/text()',
        'CastName': './text()',
        'CastUrl': './@href',
        'CastPhoto': '//*[contains(@class,"model_picture")]/img/@src',
        'SiteTags' : '//div[@class="gallery_info"]/p[@class="sceneInfo"][4]/span/a/text()',
        'Studio': ''
}
STUDIO = ["menatplay", "men at play"]
# URLS
BASE_URL = 'https://menatplay.com/'
BASE_SEARCH_URL = 'https://menatplay.com/search.php?query={0}'
...
def Start():
    ...
    HTTP.Headers['Cookie'] = 'warning=true' #Bypasses the age verification screen
    HTTP.Headers['referrer'] = 'https://menatplay.com'
# ----------------------------------------------------------------------------------------------------------------------------------
def ValidatePrefs():
...
class MenAtPlay(Agent.Movies):
    ...
    def CleanSearchString(self, myString):
        ...
        # MenAtPlay seems to fail to find Titles which have invalid chars in them, split at first incident and take first split, just to search but not compare
        ...
        while morePages:
            ...
            for title in titleList:
                ...
                try:
                    ...
                    siteStudio = 'Men at Play'
                    siteReleaseDate = title.xpath(XML_PATHS['ReleaseDate'])[0].strip()
                    siteTitle = FILMDICT['Title'].lower()
                    log(LOG_BIGLINE)
                except Exception as e:
                ...
                try:
                    utils.matchReleaseDate(siteReleaseDate, FILMDICT)
                    log(LOG_BIGLINE)
                except Exception as e:
                    log('SEARCH:: Error getting Site Release Date %s (%s)',siteReleaseDate, e)
                    log(LOG_SUBLINE)
    ...
    def update(self, metadata, media, lang, force=True):
        ...
        # Process Genres
        log(LOG_SUBLINE)
        log('UPDATE:: Current genres: %s', len(metadata.genres))
        ...
        try:
            if XML_PATHS['SiteArts']:
                ...
                for index, image in enumerate(htmlimages):
                    ...
                    if index == ((len(htmlimages) % 2) +1) and len(metadata.posters) == 0:     # processing posters
        ...
        #if not synopsis:
        log('UPDATE:: Current summary : %s', metadata.summary)
