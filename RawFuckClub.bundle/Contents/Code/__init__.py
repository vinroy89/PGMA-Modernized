...
PLUGIN_LOG_TITLE = 'RawFuckClub'
LOG_BIGLINE = '------------------------------------------------------------------------------'
LOG_SUBLINE = '      ------------------------------------------------------------------------'

# Preferences
DELAY = 1                         # Delay used when requesting HTML, may be good to have to prevent being banned from the site
...
CROPPER = r'CScript.exe "{0}/Plex Media Server/Plug-ins/RawFuckClub.bundle/Contents/Code/ImageCropper.vbs" "{1}" "{2}" "{3}" "{4}"'
THUMBOR = Prefs['thumbor'] + "/0x0:{0}x{1}/{2}"

XML_PATHS = {
        'Search': '//div[contains(@class,"last-update-item")]/descendant::div[contains(@class,"last-update-item-inner")]',
        'SearchTitle': './div[contains(@class,"last-update-title")]/a/text()',
        'SearchStudio': './div[contains(@class,"browse-channel-name")]/a/@title',
        'SearchUrl': '//div[contains(@class,"last-update-title")]/a/@href',
        'SiteTitle': '//div[@class="container-fluid"]//h2[1]/text()',
        'SiteSummary': '//p[@class="watch-description"]', #not the text but the XML element
        'ReleaseDate' : '//p[@class="watch-published-date"]/text()',
        'SiteArts': '//div[contains(@class,"watch-slide")]/a/@href',
        'CastMembers': '//div[@class="watch-badges"]/div[@class="tag-badges"]/a/span[contains(@class,"badge-primary")]/..',
        'CastMembersName': '//div[@class="watch-badges"]/div[@class="tag-badges"]/a/span[contains(@class,"badge-primary")]/../span/text()[normalize-space()]',
        'CastName': './span/text()',
        'CastUrl': './@href',
        'CastPhoto': '//img[contains(@class,"model-image")]/@src',
        'SiteTags': '//div[@class="watch-badges"]/div[@class="tag-badges"]/a/span[contains(@class,"badge-secondary")]/text()',
        'Studio': ''
}

# URLS
BASE_URL = 'https://www.rawfuckclub.com'
BASE_SEARCH_URL = 'https://www.rawfuckclub.com/vod/RFC/browse.php?search={0}'
BASE_VIDEO = 'https://www.rawfuckclub.com/video/{0}'
SITE_STUDIOS = ['Raw Fuck Club', 'Dark Alley Media']
...
def Start():
    ...
    HTTP.Headers['Referrer'] = BASE_URL + '/'
...
class RawFuckClub(Agent.Movies):
    ...
    def search(self, results, media, lang, manual):
        ...
        if FILMDICT['ClipNo']:
            ...
            try:
                ...
                return
            # Studio Name
            for siteStudio in SITE_STUDIOS:
                try:
        ...
        while morePages:
            ...
            for title in titleList:
                # Site Entry
                try:
                    log('SEARCH:: Looking for title %s', title)
    ...
    def update(self, metadata, media, lang, force=True):
        ...
        genreList = utilsGenres.getGenres(genreList)
        genreList.sort()
        ...
        #2d   Date and originally available
        if XML_PATHS['ReleaseDate'] and not metadata.originally_available_at:
            xpath = html.xpath(XML_PATHS['ReleaseDate'])
            log('UPDATE:: Getting date from site')
            if xpath:
                ...
                if (release_date):
                    release_date = html.xpath(XML_PATHS['ReleaseDate'])[0].replace("Published on","").strip()
                    log('UPDATE:: Date fetched %s', release_date)
                    date_original = Datetime.ParseDate(release_date).date()
                    metadata.originally_available_at = date_original
                    metadata.year = metadata.originally_available_at.year
                else:
                    log('UPDATE:: No date fetched')
